...
function package_managers::npm::_handle_install_pipefail() {
	...
	message=$(
		cat <<-EOF
			...
			the build ran out of disk space). This is not a problem with
	...
	if grep -qiE 'npm (ERR!|error) code ERESOLVE($| )' "${log_file}"; then
		...
		__failure["message"]=$(
			cat <<-EOF
				...
				If that is not possible, a temporary solution is to set an environment variable
				with \`scalingo --app my-app env-set npm_config_legacy_peer_deps=true\`. This
				should be used with caution as ignoring peer dependency conflicts can lead to
