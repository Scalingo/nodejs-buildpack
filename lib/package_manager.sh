...
function package_manager::run_script_command() {
	...
	if ! { "${command[@]}" 2>&1 | tee "${log_file}" | output::indent; }; then
		...
		if [[ "${tool_exit}" -eq 0 ]]; then
			...
			# A scalingo-prebuild/build/scalingo-postbuild/scalingo-cleanup script can shell out to git
...
# Emits the catch-all failure for an app lifecycle script (scalingo-prebuild/build/
# scalingo-postbuild/scalingo-cleanup) that exited non-zero for a reason none of the classifiers at
...
function package_manager::_handle_unknown_build_script_failure() {
	...
	failure["message"]=$(
		cat <<-EOF
			...
			One of your app's lifecycle scripts (scalingo-prebuild, build, scalingo-postbuild,
			or scalingo-cleanup) failed. This is usually caused by your application code or its
...
# the command run at this call site is the app's own scalingo-prebuild/build/scalingo-postbuild/
# scalingo-cleanup script, so these matchers key on Node.js/tool-level signals rather than a
...
function package_manager::_run_build_if_present() {
	...
	if [[ "${script}" == "ng build" ]]; then
		output::warning <<-EOF
			"ng build" detected as build script. We recommend you use \`ng build --prod\` or add \`--prod\` to your build flags.
		EOF
	fi
...
function package_manager::run_prebuild_script() {
	...
	has_scalingo_prebuild_script=$(utils::package_json::has_script "${build_dir}/package.json" "scalingo-prebuild")

	if [[ "${has_scalingo_prebuild_script}" == "true" ]]; then
		output::step "Prebuild"
		package_manager::_run_if_present "${build_dir}" 'scalingo-prebuild'
	fi
}
...
function package_manager::run_build_script() {
	...
	local has_scalingo_build_script

	has_build_script=$(utils::package_json::has_script "${build_dir}/package.json" "build")
	has_scalingo_build_script=$(utils::package_json::has_script "${build_dir}/package.json" "scalingo-postbuild")
	if [[ "${has_scalingo_build_script}" == "true" ]] && [[ "${has_build_script}" == "true" ]]; then
		output::info "Detected both \"build\" and \"scalingo-postbuild\" scripts"
		package_manager::_run_if_present "${build_dir}" 'scalingo-postbuild'
...
function package_manager::run_cleanup_script() {
	...
	has_scalingo_cleanup_script=$(utils::package_json::has_script "${build_dir}/package.json" "scalingo-cleanup")

	if [[ "${has_scalingo_cleanup_script}" == "true" ]]; then
		output::step "Cleanup"
		package_manager::_run_if_present "${build_dir}" 'scalingo-cleanup'
	fi
}
